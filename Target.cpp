...
Target::Target(QString filename)
{
    loadTargetFromFile(filename);
}
Target::~Target() {}
...
inline int Target::index(int X, int Y, int Z){
...
bool Target::loadTargetFromFile(QString filename)
{
    ...
    int pSizeX, pSizeY, pSizeZ, pTSize;
    pSizeX = sizeX;
    pSizeY = sizeY;
    pSizeZ = sizeZ;
    pTSize = tSize;
    ...
    for (int k=0; k < sizeZ; k++){
        for (int j=0; j < sizeY; j++){
            for (int i=0; i < sizeX; i++){
                ...
                if (objPresence == 1){
                    targetIndicesList.push_back(indx);
                }
            }
        }
    }
...
void Target::setTargetIndicesList(){
    targetIndicesList.clear();

    for (int indx=0;indx<tSize;indx++){
        if (target[indx]==1){
           targetIndicesList.push_back(indx);
        }
    }
